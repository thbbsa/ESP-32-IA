...
# Pillow (opcional, recomendado para JPEG). Se não tiver, usaremos PhotoImage com PNG.
try:
...
# pyserial (obrigatório para comunicação com ESP32)
try:
    import serial
...
QR_LOCAL_FILE = "qr.png"  # se for JPEG, use Pillow; com PhotoImage só PNG/GIF
...
class SidePanel:
    ...
    def __init__(self, parent, exit_cb):
        ...
        tk.Label(self.frame, text="Jarvis – Assistente de voz",
                 bg="#050509", fg="#FFFFFF",
    ...
    def _build_projeto(self):
        ...
        texto = (
            "Jarvis é uma assistente de voz mais acessível e personalizável, "
            "construída com ESP32-S3 e o firmware Xiaozhi.\n\n"
            "Benefícios:\n"
            "• Comandos naturais por voz e respostas geradas por IA;\n"
            "• Integração com dispositivos do ambiente (luzes, sensores, relés);\n"
            "• Adaptação ao contexto (escola, casa, empresa).\n\n"
            "Hardware do protótipo:\n"
            "• ESP32-S3 com Xiaozhi;\n"
            "• Microfone, amplificador de áudio e alto-falante.\n\n"
        )
    ...
    def _build_equipe(self):
        ...
        membros = [
            "• Josiel de Souza – Desenvolvimento e integração ESP32",
            "• Thiago – Suporte de software / testes",
            "• Cauã – Montagem de hardware e apoio técnico",
        ]
        for m in membros:
    ...
    def _build_qr(self):
        ...
        tk.Label(self.page_qr, text="Aponte a câmera para o QR Code:",
                 bg="#050509", fg="#CFD8DC",
                 font=("Segoe UI", 13), anchor="nw", wraplength=380
                 ).pack(anchor="nw", pady=(0, 8))
        ...
        # Parâmetros de tamanho máximos para caber bem no painel
        max_w = 300  # largura máxima
        max_h = 300  # altura máxima

        if PIL_AVAILABLE:
            try:
                img = Image.open(qr_path)  # PNG/JPEG
                # calcula a proporção para limitar a largura/altura
    ...
    def _qr_photoimage_fallback(self, parent, qr_path, max_w, max_h):
        try:
            ...
            # Reduz usando subsample (inteiro). Calcula fator mínimo necessário.
            # OBS: subsample só aceita inteiros; escolhemos um fator para caber em max_w/max_h
    ...
    def _build_config(self):
        tk.Label(self.page_config, text="Configurações",
...
class StatusBar:
    ...
    def set_estado(self, estado: str):
        self.estado = estado
        txt = ("Jarvis está ouvindo" if estado == "listening"
               else "Jarvis está respondendo" if estado == "speaking"
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Jarvis – Assistente de voz (ESP32-S3 + Xiaozhi)")
    ...
    def _montar_serial_ui(self, parent):
        ...
        tk.Label(frame, text="Conexão serial", bg="#050509", fg="#FFFFFF",
